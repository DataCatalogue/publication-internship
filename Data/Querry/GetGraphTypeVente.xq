...
for $i in $u
  ...
  let $c := $b//subfield[@code='b']/text()|$b//mxc:subfield[@code='b'][1]/text()
  return $c
};
...
declare function local:Graph($data,$x,$color)
{
  ...
  let $colors := color:RGBGradient(map:size($dataGraph),$color)
  for $i in 1 to map:size($dataGraph)
    ...
    return   (<rect x="{$x}%" y="{$before}%" width="10%" height="{$now}%" style="stroke:black; fill:{$colors[$i]};stroke-width:1px" />,
    if ($now >4 ) then(
...
return <svg xmlns="http://www.w3.org/2000/svg" width= "100%" height="100%">{local:Graph($bnf,0,'red'),local:Graph($inha,20,'blue'),local:Graph($all,40,'magenta')}</svg>